...
class ResponseModelTest {
    ...
    fun testBuilder_withAllArguments() {
        val expected =
            ResponseModel(statusCode, message, headers, cookies, body, timestamp, mockRequestModel)
    ...
    fun testBuilder_withMandatoryArguments() {
        val expected =
            ResponseModel(statusCode, message, mapOf(), mapOf(), body, timestamp, mockRequestModel)
        val result = ResponseModel.Builder(mockTimestampProvider)
            .statusCode(statusCode)
            .message(message)
            .body(body)
    ...
    fun testBuilder_cookiesShouldBeEmpty_whenNoSetCookiesArePresent() {
        ...
        val expected =
            ResponseModel(statusCode, message, headers, mapOf(), null, timestamp, mockRequestModel)
    ...
    fun testBuilder_cookiesAreSetWithHeaders_setCookieCaseInsensitive() {
        ...
        val cookies = mapOf(
            "cdv" to HttpCookie.parse("Set-Cookie: cdv=AAABBB;Path=/;Expires=Fri, 20-Sep-2019 14:30:24 GMT")
                .first(),
            "s" to HttpCookie.parse("Set-Cookie: s=ASDF1234").first(),
            "UserID" to HttpCookie.parse("Set-Cookie: UserID=JohnDoe; Max-Age=3600; Version=1")
                .first()
        )

        val expected =
            ResponseModel(statusCode, message, headers, cookies, null, timestamp, mockRequestModel)
    ...
    private fun <K, V> Map<K, V>.wrapValuesInList(): Map<K, List<V>> =
        this.mapValues { listOf(it.value) }
